    ...
    await axios.post('https://itgg.herokuapp.com/discord/coins', get_all)
      .then((req) => {
        ...
        if (getres.success.length > 1) {
          interaction.reply(`ยินดีด้วยกับ ${getres.success.length} คน ที่ได้รับ ${score} Token! จากกิจกรรม __${getevent}__`)
        } else if (getres.success.length == 1) {
          interaction.reply(`ยินดีด้วยกับ <@${getres.success}> ที่ได้รับ ${score} Token! จากกิจกรรม __${getevent}__`)
        }
        else {

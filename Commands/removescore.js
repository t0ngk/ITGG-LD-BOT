    ...
    await axios
      ...
      .then((req) => {
        ...
        if (getres.success.length > 1) {
          interaction.reply(
            `${getres.success.length} คน ถูกตัดคะแนน ${score} Token! เพราะว่า __${getcause}__`
          );
        } else if (getres.success.length == 1) {
          interaction.reply(
            `<@${getres.success}> ถูกตัดคะแนน ${score} Token! เพราะว่า __${getcause}__`
          );
        } else {
        ...
        getres.fail.forEach((userid) => {
          interaction.channel.send(
            `🛑 <@${userid}> คุณยังไม่ได้ยืนยันตัว กณุณายืนยันตัว(__**/auth**__) แล้วรีบแจ้ง GG Admin โดยด่วน🛑 `
          );
        });
      })
      .catch((err) => {

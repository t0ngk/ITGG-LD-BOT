  ...
  await axios
    ...
    .then((res) => {
      ...
      const sorted_leader = res.data.sort(function (a, b) {
        return parseFloat(b.coin) - parseFloat(a.coin);
      });
      const leaderboard = new MessageEmbed()
        ...
        .addFields(
          {
            name: `🏅 ${sorted_leader[0].name} : ${sorted_leader[0].coin} Token!`,
            value: "==============",
          },
          {
            name: `🥈 ${sorted_leader[1].name} : ${sorted_leader[1].coin} Token!`,
            value: "==============",
          },
          {
            name: `🥉 ${sorted_leader[2].name} : ${sorted_leader[2].coin} Token!`,
            value: "==============",
          },
          {
            name: `🐢 ${sorted_leader[3].name} : ${sorted_leader[3].coin} Token!`,
            value: "==============",
          }
        )
        .setTimestamp()
        ...
        sorted_leader.forEach((info) => {
          if (info.name == get_gate) {
            const solo_lead = new MessageEmbed()
              ...
              .addFields({
                name: `${sec_emoji[get_gate]} ${info.name} : ${info.coin} Token!`,
                value: "==============",
              })
              .setTimestamp()
              .setFooter("🗨️🤖 : อัพเดจเมื่อ");
